...
- **CTA 버튼 문구**: "위험성 평가하기" → "사용 신청하기" (Hero, NavigationBar, StickyCTA)
- **신청하기 버튼**: 모든 신청하기 버튼에서 AIR 로고 제거 (텍스트만 "신청하기" 표시)
- **사용 신청 클릭 시**: 모달 없이 `/form` 페이지로 바로 이동, CTAModal 제거
- **Footer**: MISO 로고 `miso-logo.svg` → `52g_logo.png` (52g 로고)
- **Footer 배너**: 클릭 시 `https://www.miso.gs/` 로 이동
- **StickyCTA 문구**: "GS가 만든 AI기반 위험성 평가서 솔루션" + AIR 로고 + "지금 바로 시작하세요!" → "GS가 만든 AI기반 위험성 평가 솔루션, 지금 바로 신청하세요!" (로고 삭제)
- **StickyCTA 모바일**: 버튼과 X 닫기 아이콘 오른쪽 정렬 (`self-end`)
...
- **[무상 배포 대상] 콜아웃**:
  - "공정안전관리(PSM) S/M 등급 중 100인 이하 사업장" (1줄), "(중부지청 중대재해 방지센터 관할)" (2줄), "(무상 배포 기간 : 2026. 02. 24 ~ 12.31)" (3줄)
  - 흰색 배경 + 파란 테두리 스타일 유지
- **100인 이하 사업장**: 체크박스 → 예/아니오 라디오 선택, API·Sheets는 boolean 유지
- **아니오 선택 시**: 콜아웃으로 "이 서비스는 100인 이하 사업장을 대상으로..." 안내 (흰 배경·좌측 강조선·#5541f6 강조 문장)
- **필수 동의**: 100인 이하 여부 + 개인정보 수집·이용 동의를 하나의 카드 섹션으로 통합
- **선택 항목 삭제**: "AIR의 어떤점이 마음에 드셨나요?" 복수 선택 항목 제거
- **폼 제출 버튼**: AIR 로고 제거, "신청하기" 텍스트만 표시

### ✅ 메인 페이지 뉴스–푸터 사이 섹션

- **ServiceIntroCardSection 추가**: 뉴스(PressSection)와 Footer 사이 흰색 배경 카드 섹션
- **문구**: "본 서비스는 (주)GS가 상생 협력의 가치를 담아, 중소기업의 AI 기반 안전관리를 보다 체계적으로 구축할 수 있도록 돕는 사회공헌활동입니다." / "한정된 자원으로 인해 이번 차수에는 일부 사업장을 우선 선정하여 무상 보급하고 있습니다."
- **반응형**: 모바일 4줄 구조·폰트 크기 조정, 데스크톱 레이아웃 유지

### ✅ 연락처·링크 변경

- **Contact 이메일**: `jhshim@gs.co.kr` → `miso@52g.team` (Footer, Contact 페이지)
- **소개 자료 다운로드**: 링크를 `https://drive.google.com/file/d/1vgKZURPlNwJjIvQiMUZO1Vy3OXt2lGYX/view?usp=drive_link` 로 변경 (Contact 페이지)

### ✅ 보도자료 (PressSection) API

- **중복 제거**: 제목 정규화 후 유사/중복 기사는 첫 번째만 노출 (같은 사건 다룬 여러 언론사 기사 정리)
- **기사 소스 제거**: 다음뉴스·헤럴드경제·아이코리아데일리 3개 URL 제거
